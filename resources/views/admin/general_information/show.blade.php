...
@section('content')
    ...
    <div class="w-[90rem] h-full p-[2rem] border mx-auto rounded-xl shadow-2xl mb-[4rem] text-[#525456]">
        ...
        <div class="flex justify-between mt-[1rem] text-xl ml-[4rem] mr-[4rem]">
            ...
            <div class="">
                ...
                <div
                    ...
                    @foreach (json_decode($general_information->assess_patient_risk->risk) as $index => $risk)
                        <p>{{ $risk }} @if (isset(json_decode($general_information->assess_patient_risk->risk_detail)[$index]))
                                {{ json_decode($general_information->assess_patient_risk->risk_detail)[$index] }}
                            @endif
                        </p>
                    @endforeach
                </div>
        ...
        <a class="flex justify-center mt-[1rem] ml-[4rem] mr-[4rem]" href="{{ url('/admin/general_information') }}">
            <div class="bg-[#8fd3ff] w-[20rem] text-[#00609e] rounded-lg hover:bg-[#b8e3ff] p-4 text-xl flex justify-center gap-1 ">
                <button class="flex items-center gap-1">
                    <svg class="w-8 h-8  text-center" aria-hidden="true"
