...
@section('content')
    @php
        $headers = [
            'ผู้ป่วยทั่วไป' => 'ระบุโรค',
            'ผู้ป่วยเรื้อรัง' => 'ระบุโรค',
            'OSCC' => 'ระบุประเภท',
            'ผู้ติดสารเสพติด' => 'ระบุ',
            'ผู้ป่วยจิตเวช' => 'ระบุอาการ',
            'ผู้พิการ' => 'ระบุประเภท',
        ];

        $headerText = $headers[$general_information->finding_facts->facts_target_group] ?? null;
    @endphp
    ...
    <div class="w-[90rem] h-full p-[2rem] border mx-auto rounded-xl shadow-2xl mb-[4rem] text-[#525456]">
        ...
        @if ($headerText)
            <div class="flex justify-between items-center gap-2 text-xl mt-[2rem] ml-[4rem] mr-[4rem]">
                ...
                <div class="">
                    ...
                    @if ($headerText)
                        <h1 class="font-bold text-black">{{ $headerText }}</h1>
                        <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[15rem] pl-[1rem] pr-[1rem]">
                            {{ $general_information->finding_facts->facts_target_group_detail }}
                        </p>
                    @endif
        ...
        @else
            <div class="flex justify-between items-center gap-2 text-xl mt-[2rem] ml-[4rem] mr-[4rem]">
                ...
                @if ($general_information->finding_facts->receive == 'หน่วยงานภายใน')
                    <div class="">
                        <h1 class="font-bold text-black">การรับเรื่อง</h1>
                        <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[18.8rem] pl-[1rem] pr-[1rem] ">
                            {{ $general_information->finding_facts->receive }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="">
                ...
                @elseif($general_information->finding_facts->receive == 'หน่วยงานภายนอก')
                    ...
                    <div class="">
                        <h1 class="font-bold text-black">กลุ่มเป้าหมาย/ประเภทผู้ใช้บริการ</h1>
                        <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[27rem] pl-[1rem] pr-[1rem] ">
                            {{ $general_information->finding_facts->facts_target_group }}</p>
                    </div>
                @else
                    <div class="">
                        <h1 class="font-bold text-black">การรับเรื่อง</h1>
                        <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[38.7rem] pl-[1rem] pr-[1rem] ">
                            {{ $general_information->finding_facts->receive }}</p>
                    </div>
        ...
        <div class="flex justify-between items-center mt-[1rem] text-xl ml-[4rem] mr-[4rem] ">
            ...
            @if ($general_information->finding_facts->informant === 'ผู้อื่น')
                <div class="">
        ...
        <div class="flex justify-between items-center mt-[1rem] text-xl ml-[4rem] mr-[4rem]">
            ...
            @php
                $facts = $general_information->finding_facts ?? null;
            @endphp

            @if ($facts && $facts->source_of_income === 'จากผู้อื่น')
                <div>
                ...
                <div>
                    <h1 class="font-bold text-black">แหล่งที่มาของรายได้</h1>
                    <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[10rem] pl-[1rem] pr-[1rem]">
                        {{ $facts->source_of_income }}</p>
                </div>
                <div>
                    <h1 class="font-bold text-black">ระบุ</h1>
                    <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[10rem] pl-[1rem] pr-[1rem]">
                        {{ $facts->source_of_income_detail }}</p>
                </div>
                @if ($facts->having_debt === 'มี')
                    <div>
                    ...
                    <div>
                        <h1 class="font-bold text-black">แหล่งหนี้สิน</h1>
                        <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[10rem] pl-[1rem] pr-[1rem]">
                            {{ $facts->source_of_debt }}</p>
                    </div>
                @endif
            @elseif($facts && $facts->source_of_income === 'จากการทำงาน')
                <div>
                    <h1 class="font-bold text-black">รายได้เฉลี่ยต่อเดือน</h1>
                    <p class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg w-[25rem] pl-[1rem] pr-[1rem]">
                        {{ $facts->income }}</p>
        ...
        <div class="flex  items-center mt-[1rem] text-xl">
            <div class="ml-[4rem]">
                ...
                <p
                    class="bg-[#f4f4fc] border border-[#a3a0df] p-2 rounded-lg mt-[1rem] w-[78rem] max-w-[78rem] pl-[1rem] pr-[1rem] break-words">
                    {{ $general_information->social_informations->conditions_of_problems_found }}
                </p>
            </div>
        </div>
        <div class="flex  items-center mt-[1rem] text-xl">
        ...
        <div class="flex justify-between items-center mt-[1rem] text-xl ml-[4rem] mr-[4rem]">
            ...
            <div class="">
                ...
                <div
                    ...
                    <p>{{ $general_information->family_assessments->result }}</p>
                    @if ($general_information->family_assessments->result === 'มี')
        ...
        <div
            ...
            @if (
                !empty($general_information->family_tree_and_house_map->family_tree) &&
                    !empty($general_information->family_tree_and_house_map->house_map))
                <div class="pl-[2rem]">
                    <h1 class="font-bold text-center text-black">รูปผังครอบครัว</h1>
                    <img class="w-[30rem] border-2 border-black mt-[1rem]"
                        src="{{ $general_information->family_tree_and_house_map->family_tree }}" alt="รูปผังครอบครัว">
                </div>
                <div class="pr-[2rem]">
                    <h1 class="font-bold text-center text-black">รูปเเผนที่บ้าน</h1>
                    <img class="w-[30rem] border-2 border-black mt-[1rem]"
                        src="{{ $general_information->family_tree_and_house_map->house_map }}" alt="รูปแผนที่บ้าน">
                </div>
            @elseif (!empty($general_information->family_tree_and_house_map->family_tree))
                ...
                <div class="pr-[2rem]">
                    <h1 class="font-bold text-center text-black">รูปเเผนที่บ้าน</h1>
                    <p class="text-[#f7ab2f]">ไม่มีรูปผังเผนที่บ้าน!!</p>
                </div>
            @elseif (!empty($general_information->family_tree_and_house_map->house_map))
                <div class="pl-[2rem]">
                    <h1 class="font-bold text-center text-black">รูปผังครอบครัว</h1>
                    <p class="text-[#f7ab2f]">ไม่มีรูปผังครอบครัว!!</p>
                </div>
                <div class="pr-[2rem]">
                    <h1 class="font-bold text-center text-black">รูปเเผนที่บ้าน</h1>
                    <img class="w-[30rem] border-2 border-black mt-[1rem]"
